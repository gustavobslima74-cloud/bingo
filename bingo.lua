...
local function normalizeNumber(value)
    ...
    if direct and direct % 1 == 0 and direct >= 1 and direct <= 75 then return direct end
...
glow.Position = UDim2.new(0, 16, 0.5, -278)
...
-- Progresso
makeLabel(content, "PROGRESSO DE MARCACAO", UDim2.new(1, 0, 0, 12), UDim2.fromOffset(2, 170), Enum.Font.GothamBold, 8, THEME.TextDim)
local progressBar = Instance.new("Frame")
progressBar.Size = UDim2.new(1, 0, 0, 7)
progressBar.Position = UDim2.fromOffset(0, 185)
...
local progressText = makeLabel(content, "0 marcacoes enviadas", UDim2.new(1, 0, 0, 12), UDim2.fromOffset(2, 196), Enum.Font.Gotham, 9, THEME.TextDim)
...
makePill(218, "MARCACAO AUTOMATICA", State.AutoDaub, function(on)
    State.AutoDaub = on
...
makePill(264, "BINGO AUTOMATICO", State.AutoClaim, function(on)
    State.AutoClaim = on
...
local function makeActionButton(x, w, text, callback, accent)
    ...
    btn.Position = UDim2.fromOffset(x, 314)
...
logPanel.Position = UDim2.fromOffset(0, 356)
...
-- CORREÇÃO DA MINIMIZAÇÃO (Fundo, borda e brilho somem totalmente)
connect(minimize.Activated, function()
    minimized = not minimized
    content.Visible = not minimized
    
    -- Deixa o fundo principal 100% transparente quando minimizado
    main.BackgroundTransparency = minimized and 1 or 0
    glow.Visible = not minimized
    
    -- Oculta a bordinha principal (UIStroke) do container
...
-- Monitoramento de eventos do jogo (BingoRemotes)
